    ...
    fetch('https://api.openweathermap.org/data/2.5/weather?q=' + userSearch.value +'&units=imperial&appid=cb4022af9ee9d0ebd691e41110e4c85b')
        ...
        .then(function(data) {
            ...
            coordinates.forEach(value => {
                window.localStorage.setItem(userSearch.value + 'coords', JSON.stringify(coordinates));
            });     
            (currentCityCond[1].children[0]).innerHTML = '<img src="http://openweathermap.org/img/wn/' + data.weather[0].icon + '@2x.png"></img>';;        
            (currentCityCond[1].children[1]).innerHTML = "Temp: " + data.main.temp+ "&deg; F";
            (currentCityCond[1].children[2]).innerHTML = "Wind: " + data.wind.speed+ " MPH";
            (currentCityCond[1].children[3]).innerHTML = "Humidity: " + data.main.humidity+ " %";
            getFiveDay();                       
  })  
}

function getFiveDay() {
    var coordinates = JSON.parse(localStorage.getItem(userSearch.value + 'coords'));
...
function searchCity(event) {
    ...
    $(".cities").append('<button class= "list-group-item rounded-3 py-3">'+userSearch.value.toUpperCase()+ '</button>') 
...
function generateButtons() {
    ...
    cityArr.forEach(Element => {
        console.log(Element);
        if (Element) {        
      $(".cities").append('<button class="list-group-item rounded-3 py-3 ctybtn">'+Element+ '</button>')
        }        
    })  
};
...
function setStorage(){
    ...
    console.log(cityArr.indexOf(userSearch.value.toUpperCase))
    return;
